...
public class ProcessingContextTest {
	...
	public void withReferenceDefinition() {
		...
		assertNotNull(context.link("one"));
		assertNotNull(context.link("One"));
		LinkDefinition link = context.link("ONE");
		assertEquals("onE", link.getName());
		assertEquals("/uri", link.getHref());
		assertEquals("a title", link.getTitle());
		assertNull(context.link("Unknown"));
	...
	public void merge() {
		...
		assertNotNull(merged.link("two"));
		assertNotNull(merged.link("one"));
	}
	...
	public void precedence() {
		...
		assertEquals("1", merged.link("one").getHref());
	}
}
